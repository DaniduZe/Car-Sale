...
namespace Car_Sale
{
    ...
    public partial class Addcar : Window
    {
        ...
        {
            //con = new SqlConnection("Data Source=DESKTOP-0UIQMQB;Initial Catalog=DB_VehicleSale;Integrated Security=True");
        ...
        private void Btn_Save_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                txt_error_Engine_No.Text = "Please!... Enter valid Engine Number!";
                txt_Engine_No.Focus();
            }
            ...
            else if (txt_Engine_Capacity.Text.Length != 10)
            {
                txt_error_Engine_Capacity.Text = "Please!... Enter valid Engine Capacity!";
                txt_Engine_Capacity.Focus();
            }

            else if (IsNullOrEmpty(txt_Interior_Color.Text) || IsNullOrWhiteSpace(txt_Interior_Color.Text) || txt_Interior_Color.Text.Any(IsSymbol) || txt_Interior_Color.Text.Any(IsNumber))
            {
                txt_error_Interior_Color.Text = "Please!... Enter valid Interior Color!";
                txt_Interior_Color.Focus();
            }
            else if (IsNullOrEmpty(txt_Body_Color.Text) || IsNullOrWhiteSpace(txt_Body_Color.Text) || txt_Interior_Color.Text.Any(IsSymbol) || txt_Interior_Color.Text.Any(IsNumber))
            {
            ...
            else
            {
                try
                {
                    ...
                    if (i == 1)
                    {
                        ...
                        {
                            MessageBox.Show("Warrenty Updating Failed");
                        }
                        con.Close();
                    }
                    else
        ...
        private void cmb_Supplier_Name_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            try
            {
                ...
                {
                    con.Open();
                    cmd = new SqlCommand("select NIC from Supplier where FirstName = '" + cmb_Supplier_Name.Text + "'", con);
                    dr = cmd.ExecuteReader();
                    while (dr.Read())
                    {
                        if (!dr.IsDBNull(0))
